...
function getBackendErrorMessage(responseBody: BackendSuccessResponse<unknown> | BackendErrorResponse | null): string {
  if (!responseBody || responseBody.success === true) {
    return '서버 템플릿 검토 확정 중 오류가 발생했습니다.';
  }

  return responseBody.error?.message || responseBody.message || '서버 템플릿 검토 확정 중 오류가 발생했습니다.';
}
...
export async function POST(
  ...
  const response = await fetch(
    createServerApiUrl(`/dms/templates/${encodeURIComponent(id)}/review-confirmation`),
