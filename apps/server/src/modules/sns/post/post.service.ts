...
export class PostService {
  ...
  async findAll(params: FindPostsDto, user: TokenPayload) {
    ...
    const filterWhere: Prisma.SnsPostWhereInput = {
    ...
    const [data, total] = await Promise.all([
      this.db.client.snsPost.findMany({
        where,
        skip,
        take: pageSize,
      ...
      this.db.client.snsPost.count({ where }),
    ]);
  ...
  async findOne(id: bigint, user: TokenPayload) {
    ...
    await this.db.client.snsPost.update({
      where: { id },
  ...
  async update(id: bigint, dto: UpdatePostDto, user: TokenPayload) {
    const existing = await this.db.client.snsPost.findUnique({ where: { id } });
    ...
    const result = await this.db.client.$transaction(async (tx) => {
      ...
      if (dto.tagNames !== undefined) {
        await tx.snsPostTag.deleteMany({ where: { postId: id } });
        for (const tagName of dto.tagNames) {
          const tag = await tx.snsTag.upsert({
            where: { tagName },
            create: { tagName },
