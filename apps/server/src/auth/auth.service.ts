...
import { TokenPayload, AuthTokens } from './interfaces/auth.interface';
  ...
  async refreshTokens(refreshToken: string): Promise<AuthTokens> {
    try {
      ...
      const payload = this.jwtService.verify<TokenPayload>(refreshToken, {
        secret: this.configService.get<string>('JWT_REFRESH_SECRET') || 'ssoo-refresh-secret-key',
      });
  ...
  private async generateTokens(payload: TokenPayload): Promise<AuthTokens> {
    const [accessToken, refreshToken] = await Promise.all([
      this.jwtService.signAsync(
        { ...payload, type: 'access' },
        {
          secret: this.configService.get<string>('JWT_SECRET') || 'ssoo-secret-key-change-in-production',
