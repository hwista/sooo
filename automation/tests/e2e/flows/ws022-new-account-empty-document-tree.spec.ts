...
async function expandFileTree(page: Page) {
  if (await page.getByText('표시할 문서가 없습니다.', { exact: true }).isVisible().catch(() => false)) {
    return;
  }
...
async function expectEmptyFileTree(page: Page) {
  ...
  await expect(page.getByText('표시할 문서가 없습니다.', { exact: true })).toBeVisible();
}
...
test.describe('WS-022 new-account empty document tree', () => {
   ...
   * @expected 첫 500은 shell/sidebar 안에 오류와 retry를 표시하고 retry click 1회는 단일 200 + [] 요청으로 복구한다.
   */
  test('keeps a real error visible in the shell and recovers empty-ready with one request per retry click', async ({ browser }) => {
    const emptyUser = requireAuthSession(emptyUserSession, 'empty user');
    const context = await browser.newContext({
      storageState: emptyUser.storageState,
    });
    ...
    try {
      await page.goto(`${DMS_BASE_URL}/`);
      await waitForDmsShell(page);
      await expect(page.getByText(/WS-022 forced file-tree failure/)).toBeVisible();
      await expect(page.getByRole('heading', { name: '문서 목록을 준비하지 못했습니다.' })).toHaveCount(0);
      ...
      await expect(page.getByText(/WS-022 forced file-tree failure/)).toHaveCount(0);
      await expect(page.getByRole('heading', { name: '문서 목록을 준비하지 못했습니다.' })).toHaveCount(0);
      await expectEmptyFileTree(page);
    } finally {
      await context.close();
