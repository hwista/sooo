...
async function waitForDmsShell(page: Page) {
  await expect(
    page.getByRole('banner').getByRole('searchbox', { name: '통합 검색' }),
  ).toBeVisible({ timeout: 15_000 });
}

async function waitForAuthenticatedShell(page: Page, navigate: () => Promise<unknown>) {
  await navigate();
  try {
...
async function performBrowserMultipartFetch(
  ...
  return page.evaluate(async ({ nextUrl, nextMultipart, nextAccessToken }) => {
    ...
    for (const [key, value] of Object.entries(nextMultipart as Record<
      string,
      string | { name: string; mimeType: string; base64: string }
    >)) {
...
test.describe('WS-021 DMS open issues regressions', () => {
  ...
  test('keeps locked preview tab title aligned with document title', async ({ browser }) => {
    ...
    try {
      ...
      const documentTab = viewerPage.getByRole('tab', { name: documentTitle, exact: true });
      await expect(documentTab).toBeVisible({ timeout: 30_000 });
   ...
   * 우선순위: P1
   *
   * @description external web URL이 설정된 storage-backed 첨부를 문서 패널에서 열 때 same-origin `/api/storage/open` navigation 이 401 없이 외부 redirect 를 반환하는지 검증한다.
   * @precondition admin 계정으로 로그인 가능한 Playwright DMS stack과 `https://storage.local` route stub이 준비되어 있어야 한다.
   * @input 격리된 테스트 root와 external web URL이 설정된 NAS provider source file 1건이 연결된 문서
   * @expected 첨부 클릭 시 popup이 열리고 `/api/storage/open` 응답이 401이 아니라 302 redirect 이며, `Location` 헤더가 `https://storage.local/...` 을 가리킨다.
   */
  test('opens external storage attachment through same-origin route without 401 regression', async ({ browser }) => {
    const admin = requireAuthSession(adminSession);
    const suffix = `${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
    const documentPath = `ws021/storage-open-${suffix}.md`;
    const documentTitle = `WS021 Storage Open ${suffix}`;
    const storageFileName = `ws021-external-storage-${suffix}.pptx`;
    const { context, page } = await newAuthenticatedPage(browser, admin);
    let originalNasEnabled = false;
    let originalNasBasePath = '';
    let originalNasWebBaseUrl = '';
    let storageSettingsChanged = false;

    try {
      await context.route('https://storage.local/**', async (route) => {
        await route.fulfill({
          status: 200,
          contentType: 'text/html; charset=utf-8',
          body: '<!doctype html><html><body>WS021 external storage open ok</body></html>',
        });
      });

      const settingsPayload = await apiRequest(page, admin.accessToken, 'GET', '/api/settings?includeRuntime=true');
      const settings = unwrapApiData<JsonObject>(settingsPayload);
      const config = isRecord(settings.config) ? settings.config : {};
      const system = isRecord(config.system) ? config.system : {};
      const storage = isRecord(system.storage) ? system.storage : {};
      const nas = isRecord(storage.nas) ? storage.nas : {};
      originalNasEnabled = nas.enabled === true;
      originalNasBasePath = typeof nas.basePath === 'string' ? nas.basePath : '';
      originalNasWebBaseUrl = typeof nas.webBaseUrl === 'string' ? nas.webBaseUrl : '';
      const nasTestBasePath = path.resolve(
        process.env.PLAYWRIGHT_OUTPUT_DIR ?? 'output/playwright',
        'ws021-nas-storage',
      );
      await apiRequest(page, admin.accessToken, 'POST', '/api/settings', {
        config: {
          system: {
            storage: {
              nas: {
                enabled: true,
                basePath: nasTestBasePath,
                webBaseUrl: 'https://storage.local',
              },
            },
          },
        },
      });
      storageSettingsChanged = true;

      const uploadPayload = await apiRequest(page, admin.accessToken, 'POST', '/api/storage/upload', {
        fileName: storageFileName,
        content: 'WS-021 external storage binary fixture',
        provider: 'nas',
      ...
      expect(uploaded.webUrl).toContain('https://storage.local/');
      ...
      const documentTab = page.getByRole('tab', { name: documentTitle, exact: true });
      await expect(documentTab).toBeVisible({ timeout: 30_000 });
      await documentTab.click();

      await openAttachmentSection(page, uploaded.name);
      const storageOpenResponsePromise = context.waitForEvent('response', {
        predicate: (response) => response.url().includes('/api/storage/open?'),
      });
      const popupPromise = context.waitForEvent('page');
      await page.getByRole('button', { name: uploaded.name }).click();
      const [popup, storageOpenResponse] = await Promise.all([popupPromise, storageOpenResponsePromise]);
      expect(storageOpenResponse.url()).toContain('/api/storage/open?');
      expect(storageOpenResponse.status()).toBe(302);
      expect(storageOpenResponse.headers().location).toContain('https://storage.local/');
      await popup.close();
      ...
      if (storageSettingsChanged) {
        await apiRequest(page, admin.accessToken, 'POST', '/api/settings', {
          config: {
            system: {
              storage: {
                nas: {
                  enabled: originalNasEnabled,
                  basePath: originalNasBasePath,
                  webBaseUrl: originalNasWebBaseUrl,
                },
              },
            },
          },
