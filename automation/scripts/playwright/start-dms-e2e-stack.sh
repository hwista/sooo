...
SHAREPOINT_STORAGE_ROOT="${PLAYWRIGHT_DMS_SHAREPOINT_ROOT:-$ROOT_DIR/.runtime/document-storage/sharepoint}"
NAS_STORAGE_ROOT="${PLAYWRIGHT_DMS_NAS_ROOT:-$ROOT_DIR/.runtime/document-storage/nas}"
...
write_local_test_env() {
  mkdir -p \
    ...
    "$ROOT_DIR/.runtime/document-ingest" \
    "$ROOT_DIR/.runtime/document-storage/local" \
...
DMS_INGEST_QUEUE_PATH=$ROOT_DIR/.runtime/document-ingest
DMS_STORAGE_LOCAL_BASE_PATH=$ROOT_DIR/.runtime/document-storage/local
