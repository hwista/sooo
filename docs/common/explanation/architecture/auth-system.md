...
### 3.1.1 브라우저 자격증명 field signature

- 공용 로그인 form은 `name="ssoo-login"`과 `autocomplete="on"`을 사용하고, 로그인 ID/비밀번호는 각각 `name="username"` + `autocomplete="username"`, `name="password"` + `autocomplete="current-password"`로 선언합니다. 저장 자격증명의 정상 자동완성은 이 표면에서 유지합니다.
- 비밀번호 재설정은 email, one-time code, new/confirm password를 서로 다른 `id/name`으로 구분하고 `email`, `one-time-code`, `new-password` token을 사용합니다. 계정 설정의 현재/새/확인 비밀번호도 별도 credential region과 field signature를 가집니다.
...
3. `pnpm --filter @ssoo/web-shell build`
4. `pnpm --filter @ssoo/web-auth build`
5. `pnpm --filter server exec tsc --noEmit`
6. `pnpm --filter web-pms exec tsc --noEmit`
7. `pnpm --filter web-sns exec tsc --noEmit`
8. `pnpm --filter web-dms exec tsc --noEmit`
9. `pnpm run codex:dms-guard` (DMS surface 변경 포함 시)
10. `node .github/scripts/check-patterns.js [변경 파일]`
11. `node .github/scripts/check-docs.js --all` (문서 변경 포함 시)
...
- **profile contract와 권한 경계**: `GET /api/users/profile`은 자기 계정 화면의 읽기 전용 `시스템 역할` 표시에 필요한 `roleCode`를 반환할 수 있지만, `userTypeCode`와 `isAdmin` 같은 legacy 권한 shortcut은 노출하지 않는다. `roleCode`는 browser `AuthIdentity`나 JWT에 저장하지 않고 UI gating/서버 인가에 사용하지 않으며, 실제 권한 판정은 DB-backed access snapshot이 소유한다.
...
- **protected app bootstrap 순서**: `useProtectedAppBootstrap`는 local storage hydration 이후 먼저 `checkAuth({ mode: 'blocking' })` 를 완료하고, 그 뒤에만 domain access hydrate, focus/visibility background check, unauthenticated redirect, protected app render를 허용한다. React Strict Mode effect replay 중 첫 effect cleanup이 실행되어도 진행 중인 initial auth Promise를 보존하고 다음 active effect가 같은 Promise의 완료 처리를 이어받아야 하며, one-shot ref 때문에 `initialAuthCheckCompleted`가 영구 미완료로 남아서는 안 된다. access API가 auth session 복원보다 먼저 실행되어 401 후 interceptor retry로 회복하는 흐름은 표준 부트스트랩 실패로 본다.
