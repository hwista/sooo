...
- 검색 입력의 목적·autofill 경계는 `packages/web-shell/src/search-input.tsx`의 `SsooSearchInput`이 소유한다. 공용 `Input`은 목적 중립으로 유지하고, header/sidebar/DataGrid/viewer와 5앱 화면별 검색·filter·lookup은 안정적인 `id/name`, accessible label, `type="search"`, intent marker와 non-credential autofill hint를 공용 계약에서 받는다. 앱은 기존 query state, GET query, API, Enter, clear, 선택 로직을 그대로 소유한다.
- `SsooSearchInput`의 2차 guard는 앱 source와 다른 native-autofill 값만, 실제 텍스트 입력 의도가 없을 때 거부한다. 따라서 URL·persisted 초기값과 사용자가 직접 입력·paste·drop·IME로 넣은 `admin`은 보존한다. 비제어 field의 `defaultValue`가 URL·탭 상태 변경으로 바뀌면 DOM과 guard 기준값도 함께 동기화한다. 구조상 검색 시설인 header/sidebar/독립 검색 페이지만 고유 label의 search landmark를 사용하고, 화면 내부 조건·lookup에는 새 form/landmark를 만들지 않는다. 공용 DataWorkspace와 PMS page filter의 동적 text renderer를 포함한 현행 36개 inventory와 신규 raw 검색 우회는 `verify:input-intent`가 강제한다.
- DMS의 통합 검색은 루트 `AppLayout`을 유지한 채 header 또는 홈 `AI 검색` 액션으로 여는 MDI 탭이다. 브라우저 회귀는 direct page navigation으로 이 상태를 대체하지 않고 실제 UI 액션을 사용하며, 입력 계약 변경이 DMS route/MDI 소유권을 함께 바꾸지 않게 한다.
...
- 브라우저 탭 아이콘은 5개 앱 모두 같은 `/ssot-icon.svg` route를 사용한다. 앱은 개별 `src/app/icon.svg`를 두지 않고, route handler에서 `getSsooAppIconResponse('{app}', request)`만 재노출한다. `/favicon.ico` 요청은 공용 Next rewrite로 `/ssot-icon.svg`에 연결해 기본 브라우저 탐색에서도 404를 남기지 않는다. favicon accent는 앱 기본 theme의 `ssooPrimary`와 런타임 `--ssoo-primary` 값을 기준으로 `packages/web-shell/src/app-icon.ts`가 생성하고, root layout의 `SsooFaviconSync`가 사용자 커스텀 theme 반영 시 favicon link를 `accent` 쿼리로 갱신한다.
...
- `Button`, `Badge`, `Card`, `Input`, `NativeSelect`, `Table`, `Textarea`
  ...
  - 공용 auth input의 trailing visibility action은 Button `authIcon` 역할 size(44×44px)를 소비하며 raw button이나 사용처 height/width recipe로 되돌리지 않는다.
...
- `SsooRegisteredMdiContentArea`, `SsooMdiContentArea`, `SsooMdiContentPane`, `SsooContentAreaSurface`, `SsooContentAreaEmptyState`, `SsooContentAreaState`
  ...
  - page route는 `contentPage`만 사용한다. `contentPage`는 `SsooContentPageTemplate` 또는 이를 감싼 domain/shared-surface/settings/handoff adapter를 거쳐야 하고, 공용 user profile/settings surface도 `@ssoo/web-auth`의 `createSsooUserSurfaceRouteContentPageElement()`를 통해 `contentPage`로 렌더링한다. 이 route-level user-surface helper는 profile route를 shared-surface content page helper로, account settings route를 `SsooSettingsPage` content page helper로 분기하므로 앱 ContentArea가 user-surface body, profile/settings tab path, page description, page tone을 직접 조립하지 않는다. profile shared-surface adapter는 기본 constrained 폭을 유지하고 `contentSurface="plain"`으로 `profile` page tone을 노출한다. account settings는 `SsooSettingsPage`가 `settings` page tone, settings index rail, settings body shell을 소유하고 `SsooUserSurfacePage`는 계정 설정 필드 body만 주입한다. `mainContentLayout`, `mainContentMaxWidth`, `mainContentSurface` 같은 raw opt-out을 사용하지 않는다. shared user profile/settings 내부 root도 `max-w-*`/`mx-auto`로 폭을 다시 정의하거나 page-level `h1`/description을 중복 렌더링하지 않는다. 저장/편집/취소 같은 page-level action은 profile route에서는 `useSsooSharedSurfacePageHeaderActions()`, account settings route에서는 `useSsooSettingsPageHeaderActions()`로 header action bridge에 등록하고, 본문 내부 page action button으로 다시 렌더링하지 않는다. canonical `/__user/*` path는 `SSOO_SHARED_USER_SURFACE_PATH_PREFIX` route-policy 상수와 5앱 middleware rewrite로 루트 셸에 연결하고, 실제 렌더링은 각 앱 ContentArea registry가 수행한다. `@ssoo/web-auth` account center resolver의 기본 profile/settings href도 `/__user/profile/me`, `/__user/settings`를 사용하고 `/profile/*`, `/settings` 입력은 `normalizeSsooUserSurfaceRouteEntryPath()`/`parseSsooUserSurfaceRouteEntry()` shared route-entry helper boundary에서 canonical path로 정규화한다. legacy `/profile/*`, `/settings` direct entry redirect는 SNS compatibility boundary에 한정하며, Admin/CRM/PMS/DMS middleware가 `/settings` 같은 도메인 route를 공용 사용자 표면으로 가로채면 안 된다. direct `/__user/*` browser entry는 각 앱 layout bootstrap에서 `parseSsooUserSurfaceRouteEntry()`/`getSsooUserSurfaceTabId()`로 canonical user-surface MDI tab을 열어야 하며, PMS/DMS 같은 탭 전용 앱도 홈/local adapter tab을 active로 유지하면 안 된다. stale route redirect는 `routeHandoffPage` adapter boundary를 통과하는 `contentPage`로만 허용한다. `legacyException` route kind는 public page assembly contract가 아니다. `shellPage` route kind와 `ShellPageContainer`는 public page assembly contract가 아니다.
...
| 2026-06-22 | SNS middleware에서 legacy `/profile/*`, `/settings` direct entry를 route-only `@ssoo/web-auth/user-surface-routing` helper로 canonical `/__user/*`로 redirect하도록 frame route gate와 검증을 강화 |
| 2026-06-22 | `@ssoo/web-auth` account center resolver 기본 profile/settings href를 canonical `/__user/profile/me`, `/__user/settings`로 고정하고 legacy `/profile/*`, `/settings` 입력은 resolver boundary에서 정규화하도록 `verify:ssoo-frame` 회귀 검증에 추가 |
| 2026-06-22 | SNS legacy `/profile/*`, `/settings` handoff parsing을 `@ssoo/web-auth` `normalizeSsooUserSurfaceRouteEntryPath()`/`parseSsooUserSurfaceRouteEntry()`로 중앙화하고 SNS layout의 앱 로컬 legacy path construction을 `verify:ssoo-frame`에서 차단 |
| 2026-06-22 | 공용 user profile/settings의 page action을 shared header action bridge에 등록하고 user-surface route/content/body 조립을 `@ssoo/web-auth` `createSsooUserSurfaceRouteContentPageElement()`로 중앙화해 앱별 `SsooUserSurfacePage` 직접 렌더링과 page metadata 조립 회귀를 `verify:ssoo-frame`에서 차단 |
