...
> 현행화: 2026-06-23 KST — AI/RAG platform 공용화가 주 작업축으로 전환됐다. `common.cm_ai_*` data plane, shared `@ssoo/types/common/ai*` 계약, DMS reference adapter, common retrieval/log/citation assembly, conversation/run audit, DMS Ask common retrieval migration, common model gateway integration이 들어갔다. 즉시 재개 문서는 `docs/common/explanation/architecture/ai-rag-platform-handoff.md`를 기준으로 한다.

> 현행화: 2026-07-08 KST — AI/RAG platform은 roadmap 전체 기준 약 78.30%이고, 서비스 rollout backlog를 제외한 central common foundation 기준은 89.26%다. provider-unavailable Docker Postgres runtime smoke, `docs/common/guides/ai-rag-runtime-runbook.md`, legacy DB 보호용 `DB_INIT_PRISMA_PUSH_MODE=auto`, AI/RAG 변경 시 `codex:preflight`/`codex:push-guard`의 `verify:ai-rag-platform`, `verify:ai-rag-central-foundation`, `verify:ai-rag-evidence-recorder` 자동 실행, provider-ready legacy/common comparison assertion, runtime smoke JSON report와 `verify:ai-rag-runtime-report` 검증, Markdown evidence summary, provider-ready evidence block artifact, temp ready report/summary/evidence-block/docs-digest self-test, `record:ai-rag-provider-ready-evidence` roadmap/handoff digest 기록, `complete:ai-rag-central-foundation` ordered completion runner와 workflow live/dry-run evidence/Docker runtime mode, provider-ready `--env-file`/compose Azure interpolation support, `verify:ai-rag-central-foundation:complete` provider-ready completion gate, artifact upload, provider mode-separated `.github/workflows/ai-rag-runtime.yml`, legacy `dms_document_embeddings` transition gate, `AiLegacyRetrieval*`/`CommonAiRetrieval*` explicit type alias, source coverage artifact 검증, adapter projection runtime validator, admin/system-override guarded AI index job operations, `AiIndexWorkerService` worker boundary, env-gated `AiIndexSchedulerService`, `/ai-index/jobs/metrics` retry backlog 관측성, PMS project/task RDB projection adapter, project/task 저장 이벤트 AI index job queue hook, 관리자용 controlled project backfill endpoint, project-scoped controlled task backfill endpoint, CRM opportunity/customer/activity RDB 원장/seed/history trigger, CRM opportunity/customer read/write service와 opportunity 저장/확정/해제/차수추가 및 customer/activity 저장 이벤트 AI index queue hook, CRM customer/activity controlled backfill endpoint, CRM customer/activity owner-aware AI projection ACL snapshot, CRM customer/activity API access guard/snapshot 1차, provider-gated AI index adapter, SNS post provider-gated AI index adapter 및 post 저장 이벤트 queue hook이 들어갔다. `/ai-index/status` coverage 기준은 DMS/CRM/PMS/SNS `registered`, Admin `missing_adapter`다. 중앙 공용 기반의 남은 선행 조건은 실제 Azure embedding deployment로 `complete:ai-rag-central-foundation` 또는 workflow `provider_mode=ready` 실행을 통과시키고, 검증된 summary/evidence block artifact 요약을 roadmap/handoff에 기록하는 것이다. CRM provider-ready evidence, SNS board/comment, Admin adapter는 공용 module을 소비하는 service rollout backlog로 분리한다.
