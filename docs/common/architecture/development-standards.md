...
코드 분석, 버그 추적, 리팩토링 시 반드시 **증거를 포함**합니다.

### 필수 사항

| 원칙 | 설명 |
|------|------|
| **증거 포함** | 모든 발견 사항에 파일 경로, 라인 번호 명시 |
| **추정 금지** | "~로 보임", "~일 것 같음" 대신 확인된 사실만 기술 |
| **불확실성 명시** | 확신 없으면 "확인 필요"로 표기 후 검증 방법 제안 |
| **영향 범위 명시** | 수정 시 영향받는 파일/모듈 목록 작성 |

### 예시

```markdown
# ❌ Bad
"ProjectService에서 문제가 있는 것 같습니다."

# ✅ Good
"ProjectService (apps/server/src/modules/pms/project/project.service.ts:L45)에서
findAll 메서드가 deleted_at 필터 없이 조회하여 삭제된 데이터도 반환됩니다.

영향 범위:
- ProjectController.findAll (L23)
- 프론트엔드 프로젝트 목록 페이지
"
```
