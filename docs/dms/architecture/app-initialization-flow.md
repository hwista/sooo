...
> **작성일**: 2026-01-29  
> **최종 업데이트**: 2026-01-29 (Phase 6 완료 반영)  
> **목적**: DMS 앱의 실제 초기화 흐름 분석 + PMS 비교  
> **상태**: ✅ Phase 6 완료, 레거시 코드 정리됨
...
│    ├─ useTreeStore: refreshFileTree() - Server Action으로 파일 트리 로드     │
...
│   │   ├── Header.tsx
│   │   ├── MainSidebar.tsx  ← 항상 펼침
│   │   ├── TabBar.tsx
│   │   └── sidebar/
│   │       └── SidebarFileTree.tsx  ← openTab만 호출
│   │
│   ├── pages/
│   │   ├── index.ts
│   │   ├── wiki/
...
│   │       └── AISearchPage.tsx
│   │
│   ├── common/              ← Phase 7에서 확장 예정
│   ├── editor/              ← 에디터 컴포넌트
│   │   ├── BlockEditor.tsx
│   │   └── EditorToolbar.tsx
│   ├── ui/                  ← shadcn/ui 컴포넌트
│   │
│   └── WikiEditor.tsx       ← Phase 7에서 editor/로 이동 예정
│
├── stores/                  ← Phase 6에서 정리됨 (4개만 남음)
│   ├── index.ts
│   ├── layout.store.ts
│   ├── tab.store.ts         ← 책갈피 기능 포함
│   ├── tree.store.ts
│   └── wiki-editor.store.ts
│
├── hooks/                   ← Phase 6에서 정리됨 (1개만 남음)
│   ├── index.ts
│   └── useEditor.ts
│
...
│       └───────────────────►└───────────────────────►└──────────────►│        │
│                                                                      │        │
│   SidebarFileTree                           getPageType() ──► pageComponents │
│        │                                                     │               │
│        │ openTab({ path: '/doc/...' })                       ▼               │
│        └─────────────────────────────────────────────► WikiViewerPage        │
│                                                              │               │
