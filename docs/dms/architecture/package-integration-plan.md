# DMS 통합 리팩터링 계획서

> 📅 기준일: 2026-01-29  
> 📌 목적: PMS 기준 DMS 프로젝트 구조 정렬 및 패키지 통합  
> 📂 완료 내역: `package-integration-completed.md` 참조

---

## 🔴 핵심 원칙

> **DMS는 모노레포를 몰라야 한다**

| 원칙 | 설명 |
|------|------|
| **독립 실행** | DMS는 GitLab에서 단독으로 clone하여 실행 가능해야 함 |
| **패키지 매니저** | DMS는 **npm** 사용 (모노레포는 pnpm이지만 DMS는 독립) |
| **모노레포 패키지 금지** | `@ssoo/types`, `@ssoo/database` 등 workspace 패키지 사용 금지 |
| **자체 완결** | 타입, API, 유틸리티 모두 DMS 내부에서 정의 |
| **외부 통신** | 필요시 HTTP API로만 통신 (현재 해당 없음) |

---

## 📊 현재 진행 상황

| Phase | 작업 | 상태 |
|-------|------|------|
| **0~2** | 기반 구조, 상태관리, UI 통합 | ✅ 완료 ([상세](./package-integration-completed.md)) |
| **3** | PMS 패턴 동기화 | ✅ 완료 |
| **4** | API 레이어 정리 | ✅ **완료** |
| **5** | 라우트 정리 | ✅ **완료** |
| **6** | 디자인 통일 (MUI 검토) | ⬜ 대기 |

> 📎 **장기 통합 분석**: PMS-DMS 패키지 공용화 분석은 [package-unification-analysis.md](./package-unification-analysis.md) 참조

---

## Phase 3: PMS 패턴 동기화 (2~3일) ✅ 완료

> **목적**: DMS를 PMS의 `pageComponents` 패턴과 동기화하여 일관성 및 통합 기반 마련  
> **시작일**: 2026-01-29  
> **완료일**: 2026-01-29

### 3.1 현재 상태 분석

#### PMS vs DMS 초기화 흐름 비교

| 단계 | PMS | DMS (현재) | 차이점 |
|------|-----|------------|--------|
| **Root Layout** | Providers (QueryClient) | 폰트 + Toaster만 | ❌ Providers 없음 |
| **Main Layout** | Auth 체크 → 로그인/AppLayout | AppLayout 직접 렌더 | ❌ Auth 없음 (필요시 추가) |
| **데이터 초기화** | API → refreshMenu() | Server Action → refreshFileTree() | ✅ 의도적 차이 |
| **ContentArea** | `pageComponents` lazy 로딩 | 조건부 분기 + children | ❌ 동적 로딩 없음 |
| **사이드바 클릭** | openTab() 만 | selectFile() + openTab() | ⚠️ loadFile() 누락 |
| **페이지 데이터** | 페이지가 자체 로드 | N/A (페이지 컴포넌트 없음) | ❌ 페이지 컴포넌트 필요 |

#### DMS 현재 문제점

```
파일 클릭 시 (SidebarFileTree)
├─ selectFile(node.path) ✅
├─ openTab({ path: '/wiki/...', ... }) ✅
└─ loadFile(node.path) ❌ 호출 누락!

⚠️ 결과: 탭은 열리지만 파일 내용이 로드되지 않음
```

#### 목표 흐름 (PMS 패턴)

```
ContentArea (PMS 패턴)
├─ pageComponents 매핑 추가
│     ├─ '/wiki': WikiHomePage (대시보드)
│     ├─ '/wiki/:path': WikiViewerPage (문서 뷰어)
│     └─ '/ai-search': AISearchPage
└─ <Suspense><PageComponent /></Suspense>

SidebarFileTree (간소화)
└─ openTab() 만 호출
     └─ WikiViewerPage가 자체적으로 loadFile() 호출
```

### 3.2 필요한 페이지 컴포넌트

| 컴포넌트 | 경로 패턴 | 역할 | 우선순위 |
|----------|-----------|------|----------|
| `WikiHomePage` | `/wiki` (Home 탭) | 대시보드, 빠른 액세스 | ⭐⭐⭐ |
| `WikiViewerPage` | `/wiki/:path` | 문서 조회/편집 + loadFile 자동 호출 | ⭐⭐⭐ |
| `AISearchPage` | `/ai-search` | AI 검색 결과 | ⭐⭐ |

### 3.3 파일 구조 계획

```
src/components/
├── pages/                          # 신규 디렉토리
│   ├── index.ts
│   ├── wiki/
│   │   ├── index.ts
│   │   ├── WikiHomePage.tsx
│   │   └── WikiViewerPage.tsx
│   └── ai/
│       ├── index.ts
│       └── AISearchPage.tsx
├── layout/
│   └── ContentArea.tsx             # pageComponents 패턴 적용
└── editor/                         # 기존 (재사용)
```

### 3.4 작업 체크리스트

**Phase 3-A: 페이지 컴포넌트 생성**
- [x] `components/pages/` 디렉토리 생성 ✅
- [x] `WikiHomePage.tsx` 생성 (Home 탭 UI 이동) ✅
- [x] `WikiViewerPage.tsx` 생성 (핵심: loadFile 호출) ✅
- [x] `AISearchPage.tsx` 생성 (AI 검색 UI 이동) ✅
- [x] 각 디렉토리 `index.ts` 배럴 export ✅

**Phase 3-B: ContentArea 리팩토링**
- [x] `pageComponents` 매핑 객체 추가 (React.lazy) ✅
- [x] Home 탭 → WikiHomePage 연결 ✅
- [x] AI 검색 → AISearchPage 연결 ✅
- [x] 문서 탭 → WikiViewerPage 연결 ✅
- [x] Suspense 로딩 UI 추가 ✅

**Phase 3-C: 정리 및 테스트**
- [x] SidebarFileTree에서 selectFile() 제거 ✅
- [ ] 기존 WikiApp.tsx, WikiSidebar.tsx 참조 정리 (Phase 4에서)
- [x] 전체 플로우 테스트 ✅

---

## Phase 4: API 레이어 정리 (1~2일) ✅ 완료

> **목적**: 서비스 구조 PMS 스타일로 통일 + API 클라이언트 구성  
> **완료일**: 2026-01-29

### 완료 내용

**apiClient.ts 확장:**
- [x] `userApi` - 로그인, 사용자 생성, 프로필 업데이트
- [x] `searchApi` - 시맨틱 검색, 텍스트 검색, 인덱싱
- [x] `uploadApi` - 파일 업로드
- [x] `aiApi` - AI 질의, Gemini 채팅

**직접 fetch 호출 제거:**
- [x] `user-store.ts` → `userApi` 사용
- [x] `SearchPanel.tsx` → `searchApi` 사용
- [x] `FileUpload.tsx` → `uploadApi` 사용
- [x] `AIChat.tsx`, `GeminiChat.tsx` → `aiApi` 사용
- [x] `AISearchPage.tsx` → `searchApi`, `aiApi` 사용
- [x] `TextSearch.tsx` → `searchApi` 사용

**추가 수정:**
- [x] `AppLayout` - children prop 제거 (PMS 패턴 동기화)
- [x] `MainSidebar` - `refreshFiles` → `refreshFileTree` 수정

---

## Phase 5: 라우트 정리 (1~2일) ✅ 완료

> 현재 기준 필수 라우트만 정리  
> **완료일**: 2026-01-29

### 완료 내용

**루트 진입점 변경:**
- [x] `/` 루트가 메인 진입점 (HOME_TAB path: `/`)
- [x] 문서 탭 경로: `/doc/{filePath}`
- [x] `/wiki` 라우트 제거

**라우트 구조:**
```
src/app/
├── layout.tsx         # Root (Toaster)
└── (main)/
    ├── layout.tsx     # AppLayout
    └── page.tsx       # 루트 페이지 (/)
```

---

## Phase 6: 디자인 통일 (장기, 점진적)

> Radix UI 기반 컴포넌트 교체 (MUI 검토)

### 작업 내용

- [ ] 필요한 Radix UI 패키지 선택 설치
- [ ] `tailwindcss-animate` 설치
- [ ] MUI 컴포넌트 → Radix/shadcn 스타일로 점진적 교체
- [ ] `@mui/x-tree-view` 유지 또는 대체 검토

---

## DMS 전용 패키지 (유지)

> DMS 도메인 특화 패키지 - 제거하지 않음

| 패키지 | 용도 | 비고 |
|--------|------|------|
| `@tiptap/*` (15개) | 리치 텍스트 에디터 | DMS 핵심 |
| `@google/generative-ai` | Gemini AI | AI 기능 |
| `@lancedb/lancedb` | 벡터 DB | RAG 검색 |
| `@mui/x-tree-view` | 트리 뷰 | 파일 탐색기 |
| `marked`, `react-markdown` | 마크다운 | 문서 렌더링 |
| `lowlight` | 코드 하이라이팅 | 에디터 |
| `turndown` | HTML→MD 변환 | 마크다운 |

---

## PMS 도입 후보 (선택적)

> 필요시 도입 검토

| 우선순위 | 패키지 | 용도 | 현재 |
|---------|--------|------|------|
| P2 | `@tanstack/react-query` | 서버 상태 | ⬜ Phase 4 |
| P2 | `axios` | HTTP 클라이언트 | ⬜ Phase 4 |
| P3 | `@tanstack/react-table` | 데이터 테이블 | ⬜ 필요시 |
| P3 | `dayjs` | 날짜 처리 | ⬜ 필요시 |

---

## 주의사항

### ❌ 하지 말아야 할 것

| 항목 | 이유 |
|------|------|
| `@ssoo/types` 연동 | DMS 모노레포 독립성 원칙 위배 |
| `@ssoo/database` 연동 | DMS 모노레포 독립성 원칙 위배 |
| `workspace:*` 의존성 | GitLab 단독 배포 불가 |

### ✅ 해야 할 것

| 항목 | 이유 |
|------|------|
| DMS 자체 타입 정의 | `src/types/`에서 독립 관리 |
| npm 공개 패키지만 사용 | 어디서든 설치 가능 |
| `src/lib/api/`로 API 호출 추상화 | 통합 시 URL만 변경 |

---

## DMS 독립 실행 가이드

### 패키지 매니저

| 프로젝트 | 패키지 매니저 |
|----------|--------------|
| 모노레포 (PMS, Server) | **pnpm** |
| **DMS** | **npm** |

### 실행 방법

```bash
# 1. DMS 폴더로 이동
cd apps/web/dms

# 2. 의존성 설치 (npm 사용)
npm install

# 3. 개발 서버 실행
npm run dev
# → http://localhost:3000/wiki
```

### 모노레포 내 실행 시

```bash
# DMS만 실행 (모노레포 루트에서 turbo 미적용)
cd apps/web/dms && npm run dev
```

---

## 변경 이력

| 날짜 | 내용 |
|------|------|
| 2026-01-29 | **루트 진입점 변경** - `/wiki` → `/`, 문서 경로 `/doc/` |
| 2026-01-29 | **Phase 5 완료** - 라우트 정리 (/ → /wiki 리디렉트) |
| 2026-01-29 | **패턴 통일** - Store 파일명 (`*.store.ts`), types/tab.ts 분리 |
| 2026-01-29 | **Phase 4 완료** - API 레이어 정리 |
| 2026-01-29 | 문서 분리 - 완료 내역 → `package-integration-completed.md` |
| 2026-01-29 | **Phase 3 완료** - pageComponents 패턴, WikiViewerPage, SidebarFileTree 단순화 |
| 2026-01-29 | **Phase 3 시작** - PMS 패턴 동기화 |
| 2026-01-29 | Phase 2 완료 - 레이아웃 + 스타일 통합 |

---

> 📌 **완료된 Phase 0~2 상세 내역**은 `package-integration-completed.md`를 참조하세요.
