...
#### Phase 2-G: 컴포넌트 재분류 + 레이아웃 시스템 (진행 중)

> PMS 레이아웃 구조를 DMS에 적용하여 일관성 확보  
> 추후 공용화 시 동일 구조로 패키지 분리 가능

##### 📊 PMS vs DMS 현재 구조 비교

| 영역 | PMS 컴포넌트 | DMS 현재 | DMS 변경 계획 |
|------|------------|----------|--------------|
| **전체 레이아웃** | `AppLayout` | `WikiApp` | `AppLayout` (PMS 동일) |
| **헤더** | `Header` | 없음 | `Header` 추가 |
| **사이드바** | `MainSidebar` (접기 지원) | `WikiSidebar` | `MainSidebar` (접기 없음) |
| **탭바** | `TabBar` | 없음 | `TabBar` 추가 (문서 탭) |
| **콘텐츠** | `ContentArea` | `WikiEditor` | `ContentArea` (WikiEditor 포함) |
| **파일트리** | `SidebarMenuTree` | `TreeComponent` | `SidebarFileTree` (파일 기반) |

##### 🎯 DMS 레이아웃 변경 사항

**1. AppLayout 구조 (PMS와 동일)**
```
┌───────────────────────────────────────────────────┐
│                    Header                          │
...
| 요소 | PMS | DMS | 비고 |
|------|-----|-----|------|
| 통합 검색 | 준비 중 (placeholder) | **AI 검색** (Gemini/RAG 드롭다운 선택) | 결과는 탭으로 표시 |
| 빠른 생성 | "새 프로젝트" | **"새 도큐먼트"** | 용어 변경 |
| 알림 | 동일 | 동일 | - |
| 프로필 | 동일 | 동일 | - |
...
| 요소 | PMS | DMS | 비고 |
|------|-----|-----|------|
| 접기 버튼 | 사이드바 접기/펼치기 | **문서 전환 드롭다운** | 위키/시스템개발문서/블로그 선택 |
| 검색 | "메뉴 검색..." | **"파일 검색..."** | placeholder 변경 |
| 즐겨찾기 | 있음 | **제외** | DMS에 불필요 |
| 열린 탭 | 열린 페이지 목록 | **열린 문서 목록** | 동일 구조 |
| 트리 | `SidebarMenuTree` (메뉴) | **`SidebarFileTree`** (파일) | 유사 구조 |

**4. AI 기능 두 가지 비교**

| 기능 | GeminiChat | AIChat (RAG) |
|------|------------|--------------|
| **API** | `/api/gemini` | `/api/ask` |
| **용도** | 일반 질문 (Gemini 직접) | 문서 기반 검색 (벡터DB+Gemini) |
| **소스 표시** | ❌ | ✅ (참고 문서 링크) |
| **대화 기록** | 단일 질답 | 히스토리 유지 |
| **상태** | `gemini-store` | 로컬 state |
| **위치** | 헤더 AI검색 드롭다운 옵션 | 헤더 AI검색 드롭다운 옵션 |

> 📌 **결정**: 두 AI 기능은 헤더 검색창 우측 드롭다운으로 선택 가능하게 함.  
> 결과는 새 탭으로 열리며, 대화 이어가기 지원 (후순위)

##### ✅ Phase 2-G 작업 목록

**Step 1: 디렉토리 구조 생성**
- [ ] `src/components/layout/` 생성
- [ ] `src/components/layout/sidebar/` 생성
- [ ] `src/components/pages/` 생성
- [ ] `src/components/pages/wiki/` 생성 (기존 wiki/ 이동)

**Step 2: 레이아웃 컴포넌트 생성 (PMS 복사 + 수정)**
- [ ] `layout/AppLayout.tsx` - PMS 복사 후 DMS용 수정
- [ ] `layout/Header.tsx` - PMS 복사 후 AI검색+새도큐먼트 적용
- [ ] `layout/MainSidebar.tsx` - 접기 없음, 문서전환 드롭다운 추가
- [ ] `layout/TabBar.tsx` - PMS와 동일 (문서 탭용)
- [ ] `layout/ContentArea.tsx` - WikiEditor/Viewer 렌더링

**Step 3: 사이드바 서브 컴포넌트**
- [ ] `sidebar/SidebarSearch.tsx` - "파일 검색..." placeholder
- [ ] `sidebar/SidebarOpenTabs.tsx` - 열린 문서 목록
- [ ] `sidebar/SidebarFileTree.tsx` - 기존 TreeComponent 기반

**Step 4: Store 추가**
- [ ] `tab-store.ts` - 탭 상태 관리 (PMS tabStore 참고)
- [ ] `sidebar-store.ts` - 사이드바 상태 (PMS 참고, 접기 제외)

**Step 5: 기존 컴포넌트 이동**
- [ ] `WikiEditor.tsx` → `pages/wiki/WikiEditor.tsx`
- [ ] `WikiSidebar.tsx` → 삭제 (MainSidebar로 대체)
- [ ] `WikiApp.tsx` → 삭제 (AppLayout으로 대체)
- [ ] `TreeComponent.tsx` → `sidebar/SidebarFileTree.tsx`로 통합
- [ ] `editor/` → `pages/wiki/editor/` 또는 `common/editor/`
- [ ] AI 컴포넌트 → `pages/ai/` (후순위)

**Step 6: 페이지 연결**
- [ ] `src/app/(main)/layout.tsx` → AppLayout 적용
- [ ] 문서 열기 → 탭으로 열리도록 변경
- [ ] 탭 클릭 → 해당 문서 표시

**Step 7: AI 검색 헤더 통합 (후순위)**
- [ ] 헤더 검색란에 AI 검색 드롭다운 추가
- [ ] GeminiChat/AIChat 선택 가능
- [ ] 결과를 새 탭으로 표시

##### 📁 최종 컴포넌트 구조 (Phase 2-G 완료 후)
...
│   ├── Header.tsx             ← AI검색, 새도큐먼트, 알림, 프로필
│   ├── MainSidebar.tsx        ← 사이드바 (접기 없음)
│   ├── TabBar.tsx             ← 문서 탭
│   ├── ContentArea.tsx        ← 탭별 콘텐츠 렌더링
│   └── sidebar/
│       ├── index.ts
│       ├── SidebarSearch.tsx  ← 파일 검색
│       ├── SidebarOpenTabs.tsx ← 열린 문서
│       └── SidebarFileTree.tsx ← 파일 트리
├── pages/                     ← 페이지별 전용 컴포넌트
│   ├── wiki/
│   │   ├── WikiEditor.tsx     ← 문서 편집기
│   │   ├── WikiViewer.tsx     ← 문서 뷰어 (읽기 전용)
│   │   └── editor/            ← Tiptap 에디터 관련
│   └── ai/                    ← AI 관련 (후순위)
│       ├── GeminiChatPage.tsx
│       └── RAGSearchPage.tsx
├── common/                    ← 재사용 가능 범용 컴포넌트
│   ├── FileUpload.tsx
│   ├── CreateFileModal.tsx
│   └── MessageModal.tsx
