...
| 카테고리 | 엔드포인트 |
|----------|------------|
| 파일 | `/api/file`, `/api/files`, `/api/file/raw`, `/api/file/serve-attachment` |
| Git | `/api/git` |
| 설정 | `/api/settings` |
...
> 주요 DMS business API는 same-origin Next route handler를 통해 `apps/server` DMS 모듈로 프록시됩니다. `/api/file/raw`, `/api/file/serve-attachment` 같은 바이너리 surface도 shared session cookie 기반 auth proxy를 거친 뒤 server 권한 검사를 다시 수행합니다.

### 5.2 운영 경계

- 브라우저는 항상 DMS origin의 same-origin route handler를 먼저 호출합니다.
- Route handler는 auth header/cookie를 upstream으로 전달하거나, 필요 시 shared session cookie로 access token을 복원한 뒤 `apps/server` DMS module로 요청을 넘깁니다.
- DMS의 다음 단계는 endpoint 추가보다 `DocumentMetadata.acl` 기반 object policy를 현재 운영 API 전체에 관통 적용하는 것입니다.
...
8. 새 탭에서는 shell과 document panel을 유지한 채, 에디터 초안이 비어 있는 동안 본문 영역에만 overlay + 로딩 문구를 표시합니다.
9. 사용자가 저장하면 원본 문서는 건드리지 않고 문서 Git 레포의 `_templates/` 하위(`_templates/system/<id>.md` 또는 `_templates/personal/<id>.md`) markdown content 와 DB metadata row 를 생성합니다. 템플릿은 문서 Git 레포 안에 포함되어 GitLab과 자동 동기화됩니다.
...
- 템플릿 전환처럼 shell이 이미 렌더된 화면은 전체 화면 gate로 가리지 않고, 본문/패널 안에서 로딩 상태를 표시합니다.
...
- `markdownRoot/_templates/system/<id>.md`
- `markdownRoot/_templates/personal/<id>.md`
- `originType`, `referenceDocuments`, `generation` 등 template control metadata 는 `dms.dm_template_m.metadataJson`

> 템플릿은 문서 Git 레포의 `_templates/` 하위에 배치되며 GitLab과 자동 동기화됩니다. 별도 `DMS_TEMPLATE_ROOT` 환경변수나 Docker volume mount 는 불필요합니다.
