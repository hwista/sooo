...
- 현재 사용자 scope의 요청이 성공하고 store가 initialized 되면 결과가 `[]`이어도 준비 완료다. 메인 shell을 렌더하고 사이드바가 `파일이 없습니다.` empty state를 표시한다.
- 네트워크/API 오류만 bootstrap 차단 화면과 `문서 목록 다시 불러오기` action을 표시한다. 성공한 빈 결과를 오류로 바꾸거나 숨은 자동 재시도를 수행하지 않는다.
