...
- verify/build 직전에는 실행 중 container, tagged image, volume을 삭제하지 않고 unused BuildKit cache와 dangling image만 정리합니다. 정리 후 Docker root의 여유 공간이 기본 8 GiB 미만이면 실제 build 전에 실패하며, Compose image build는 기본 parallel limit 1로 실행해 동시 dependency layer의 peak disk 사용량을 제한합니다. 임계값과 cache 보존량은 각각 `CI_BUILD_MIN_FREE_KB`, `CI_BUILD_CACHE_KEEP_STORAGE`, 병렬도는 `CI_BUILD_PARALLEL_LIMIT`로 조정할 수 있습니다.
