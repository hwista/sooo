...
- 실패: `4xx/5xx + { "error": "..." }`
- 예외: `/api/ask`, `/api/create`, `/api/storage/open` 은 성공 시 각각 스트림/바이너리/redirect 응답을 반환할 수 있음. `storage/open?download=1` 은 redirect 없이 same-origin binary 다운로드로 고정합니다.
...
| | `/api/collaboration/takeover/respond` | 현재 편집자의 soft lock 이동 요청 승인/거절 |
...
댓글은 문서 metadata 배열을 저장하는 방식이 아니라 server DMS module 의 `dm_document_comment_m` relation 을 직접 변경합니다. FE 는 응답의 `comments` 배열만 현재 문서 metadata projection 에 반영합니다.

권한 기준:

- 목록 조회/작성/답글: 문서 읽기 권한 필요
- 삭제/복원: 댓글 작성자 본인 또는 문서 관리자 권한 필요
- 댓글 알림: 새 댓글은 문서 소유자에게, 답글은 문서 소유자와 원 댓글 작성자에게 발송하며 작성자 본인은 수신 대상에서 제외
- 댓글 변경 실시간 반영: 공용 알림 SSE의 `dms.document-comment.changed` 도메인 이벤트를 받은 열린 문서 탭이 댓글 목록만 재조회
- 댓글 알림 읽음 처리: 대상 문서를 이미 열고 있는 경우 같은 문서 경로의 DMS 알림을 읽음 처리

### GET /api/comments

Query:

- `path` (필수): 댓글을 조회할 문서 경로
...
- `path` (필수): 댓글이 속한 문서 경로

작성자 본인 또는 문서 관리자가 댓글을 삭제할 수 있습니다. 삭제 시 댓글은 tombstone 상태로 남고, 삭제한 사용자 정보가 함께 기록됩니다.
...
복원은 해당 댓글을 삭제한 사용자만 수행할 수 있습니다. 문서 관리자는 삭제된 댓글의 감사 정보는 볼 수 있지만, 다른 사용자가 삭제한 댓글을 대신 복원하지 않습니다.
...
새 문서 런처의 AI 요약 진입은 같은 compose surface를 사용합니다. 선택한 원본 파일은 클라이언트에서 텍스트/이미지 컨텍스트로 전달하고, 문서 저장 시 `sourceFiles` 참조 첨부로 함께 업로드됩니다.
...
- SVG는 브라우저 active content surface가 될 수 있어 이미지 업로드/raw preview 대상에서 제외하고, 첨부파일로 업로드하더라도 다운로드로만 제공합니다.
...
- 최대 크기: 20MB
- 허용 확장자: `.pdf`, `.doc`, `.docx`, `.xls`, `.xlsx`, `.ppt`, `.pptx`, `.txt`, `.md`, `.csv`, `.json`, `.xml`, `.yaml`, `.yml`, `.log`, `.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, `.svg`, `.html`, `.htm`
- `.html`, `.htm`, `.svg` 는 저장은 허용하지만 same-origin binary surface에서는 항상 `Content-Disposition: attachment` 로 내려갑니다.
...
| `download` | ❌ | `1`이면 attachment, 미지정이면 inline. 단 `.html`, `.htm`, `.svg` 는 active content 방지를 위해 항상 attachment |

### Response

바이너리 (`Content-Type`, ASCII fallback + UTF-8 `filename*` 기반 `Content-Disposition`, `X-Content-Type-Options: nosniff` 헤더 포함, `Cache-Control: private, no-store`)
