...
export type SsooSectionedShellVariant =
  | 'editor_with_footer'
  | 'viewer_with_toolbar'
  | 'chat_with_footer'
  | 'search_with_toolbar';

export interface SsooSectionedShellProps {
  toolbar?: ReactNode;
  body: ReactNode;
  footer?: ReactNode;
  variant: SsooSectionedShellVariant;
...
export function SsooSectionedShell({
  toolbar,
  body,
  footer,
  variant,
  ...
  return (
    <div className={cn('flex h-full flex-col', ROOT_CLASS, preset.shellClass, className)}>
      ...
      {hasFooter ? (
        <footer className={cn(SLOT_FOOTER_BASE, borderColor, preset.footerClass)}>
          {footer}
        </footer>
      ) : null}
    </div>
