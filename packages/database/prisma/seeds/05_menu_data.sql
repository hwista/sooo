...
-- 1레벨: 대시보드, 요청, 제안, 실행, 전환
-- ============================================

-- 1. 대시보드 (메인 진입점)
INSERT INTO cm_menu_m (menu_code, menu_name, menu_name_en, menu_type, menu_path, icon, sort_order, menu_level, is_visible, is_admin_menu, description, updated_at)
...
-- 4. 실행 (계약 체결 후 프로젝트 수행)
INSERT INTO cm_menu_m (menu_code, menu_name, menu_name_en, menu_type, menu_path, icon, sort_order, menu_level, is_visible, is_admin_menu, description, updated_at)
VALUES ('execution', '실행', 'Execution', 'group', '/execution', 'Rocket', 4, 1, true, false, '계약 체결 후 프로젝트 수행', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  menu_name_en = EXCLUDED.menu_name_en,
  menu_type = EXCLUDED.menu_type,
...
-- 요청 > 요청 목록
INSERT INTO cm_menu_m (menu_code, menu_name, menu_name_en, menu_type, menu_path, icon, sort_order, menu_level, parent_menu_id, is_visible, is_admin_menu, description, updated_at)
VALUES ('request.list', '요청 목록', 'Request List', 'menu', '/request', 'List', 1, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'request'), true, false, '요청 목록 조회', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
...
VALUES ('proposal.list', '제안 목록', 'Proposal List', 'menu', '/proposal', 'List', 1, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'proposal'), true, false, '제안 목록 조회', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
...
VALUES ('execution.list', '프로젝트 목록', 'Project List', 'menu', '/execution', 'List', 1, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'execution'), true, false, '실행 프로젝트 목록 조회', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
...
VALUES ('transition.list', '전환 목록', 'Transition List', 'menu', '/transition', 'List', 1, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'transition'), true, false, '전환 목록 조회', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
...
VALUES ('admin.user', '사용자 관리', 'User Management', 'menu', '/admin/user', 'Users', 1, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '사용자 계정 관리', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
...
VALUES ('admin.role', '역할 관리', 'Role Management', 'menu', '/admin/role', 'UserCog', 2, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '역할 및 권한 관리', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  parent_menu_id = (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'),
  is_admin_menu = true,
...
VALUES ('admin.menu', '메뉴 관리', 'Menu Management', 'menu', '/admin/menu', 'Menu', 3, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '메뉴 구조 관리', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  parent_menu_id = (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'),
  is_admin_menu = true,
...
VALUES ('admin.code', '코드 관리', 'Code Management', 'menu', '/admin/code', 'Code', 4, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '공통 코드 관리', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  parent_menu_id = (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'),
  is_admin_menu = true,
...
VALUES ('admin.customer', '고객사 관리', 'Customer Management', 'menu', '/admin/customer', 'Building2', 5, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '고객사/플랜트/시스템 기준정보', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  parent_menu_id = (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'),
  is_admin_menu = true,
...
VALUES ('admin.dept', '부서 관리', 'Department Management', 'menu', '/admin/dept', 'Network', 6, 2, 
        (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'), true, true, '부서 구조 관리', CURRENT_TIMESTAMP)
ON CONFLICT (menu_code) DO UPDATE SET
  menu_name = EXCLUDED.menu_name,
  parent_menu_id = (SELECT menu_id FROM cm_menu_m WHERE menu_code = 'admin'),
  is_admin_menu = true,
