...
-- PROJECT_STATUS (4단계: 요청 → 제안 → 실행 → 전환)
insert into cm_code_m (code_group, code_value, display_name_ko, display_name_en, description, sort_order, created_at, updated_at)
values
...
('PROJECT_STATUS','execution','실행','Execution','계약 체결 후 프로젝트 수행 단계.',30,now(),now()),
...
('PROJECT_STAGE','done','완료','Done','해당 상태의 종료.',30,now(),now())
on conflict (code_group, code_value) do update
set display_name_ko=excluded.display_name_ko,
    display_name_en=excluded.display_name_en,
    description=excluded.description,
...
-- request done: accepted(수용), rejected(거부), hold(보류)
...
values
-- 요청 단계 결과
('PROJECT_DONE_RESULT','accepted','수용','Accepted','요청 수용(제안 단계 전환 대상).',10,now(),now()),
('PROJECT_DONE_RESULT','rejected','거부','Rejected','요청 거부(종료).',15,now(),now()),
-- 제안 단계 결과
('PROJECT_DONE_RESULT','won','수주','Won','계약 성사(실행 전환 대상).',20,now(),now()),
('PROJECT_DONE_RESULT','lost','실주','Lost','무산/패배.',25,now(),now()),
-- 실행 단계 결과
('PROJECT_DONE_RESULT','completed','완료','Completed','프로젝트 정상 완료(전환 단계 전환 대상).',30,now(),now()),
