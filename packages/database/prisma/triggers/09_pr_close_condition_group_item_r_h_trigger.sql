-- =========================================================
-- History Trigger: pr_close_condition_group_item_r_m -> pr_close_condition_group_item_r_h
...
BEGIN
  ...
  INTO v_history_seq
  FROM pr_close_condition_group_item_r_h
  WHERE group_code = v_record.group_code
    AND condition_code = v_record.condition_code;

  -- 히스토리 테이블에 삽입
  INSERT INTO pr_close_condition_group_item_r_h (
...
-- 기존 트리거 삭제 후 재생성
DROP TRIGGER IF EXISTS trg_pr_close_condition_group_item_r_h ON pr_close_condition_group_item_r_m;

CREATE TRIGGER trg_pr_close_condition_group_item_r_h
AFTER INSERT OR UPDATE OR DELETE ON pr_close_condition_group_item_r_m
FOR EACH ROW EXECUTE FUNCTION fn_pr_close_condition_group_item_r_h_trigger();

COMMENT ON FUNCTION fn_pr_close_condition_group_item_r_h_trigger() IS '종료조건 그룹 항목 히스토리 트리거 함수';
