-- =========================================================
-- History Trigger: pr_deliverable_group_m -> pr_deliverable_group_h
-- =========================================================
...
BEGIN
  ...
  INTO v_history_seq
  FROM pr_deliverable_group_h
  WHERE deliverable_group_id = v_record.deliverable_group_id;

  -- 히스토리 테이블에 삽입
  INSERT INTO pr_deliverable_group_h (
...
-- 기존 트리거 삭제 후 재생성
DROP TRIGGER IF EXISTS trg_pr_deliverable_group_h ON pr_deliverable_group_m;

CREATE TRIGGER trg_pr_deliverable_group_h
AFTER INSERT OR UPDATE OR DELETE ON pr_deliverable_group_m
